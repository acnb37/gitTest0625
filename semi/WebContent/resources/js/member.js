...
$("#id").on("input",function(){
    ...
    //입력된 아이디 양쪽 공백 제거를 얻어와 inputId 변수에 저장
...
$('#name').on("input",function(){
    ...
    if(regExp.test(inputName)){
        ...
        $('#checkName').text('한글로 2글자에서 5글자 사이로 작성하시오').css("color","red");
        checkObj.name=false;
    }
});
...
$('#email').on("input",function(){
    ...
    if(regExp.test(inputEmail)){
        ...
        $('#checkEmail').text('아이디 4글자 이상, 이메일 형식으로 작성하세요').css("color","red");
...
$('#pwd1').on("input",function(){
    ...
    const inputPw = $('#pwd1').val().trim();
    if(regExp.test(inputPw)){
        $('#checkPwd1').text('유효한 비밀번호 입니다.').css("color","green");
        checkObj.pwd1=true;

    }else{
        $('#checkPwd1').text('영어,숫자, 특수문자(#,-,_) 6~20글자').css("color","red");
        checkObj.pwd1=false;
...
$('.phone').on("input",function(){
    ...
    checkObj.phone2=true;
        ...
        $('#checkPhone').text("전화번호가 유효하지 않습니다.").css("color","green");
